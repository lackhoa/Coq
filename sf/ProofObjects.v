...
(* ===> ev 4 *)
Check (ev_SS _ (ev_SS _ ev_0)). (*You can leave about the details and no one cares*)
...
Definition me_ev_plus4 :=
  fun (n : nat) => fun (H : ev n) =>
    ev_SS (S (S n)) (ev_SS n H). (*You don't need the type*) 
...
Definition and_ex1 (P Q: Prop) (H: P /\ Q) :=
  match H with
  | conj HP HQ => HP
  end.

Definition and_ex2 (P Q: Prop) (H: P /\ Q) :=
  match H with
  | conj HP HQ => HQ
  end.

Definition conj_fact_aux (A B C: Prop) (HAB: A /\ B) (HBC: B /\ C) :=
  match HAB with
  | conj HA HB => match HBC with
    | conj _ HC => conj HA HC
    end
  end.  
...
Definition or_comm_aux (P Q: Prop) (H: P \/ Q) :  Q \/ P :=
  match H with
...
(*View -> Diplay all basic low-level contents to see the "fake equality" we defined*)
Lemma leibniz_equality : forall (X : Type) (x y: X),
  x = y -> forall P:X->Prop, P x -> P y. 
Proof. intros. inversion H. rewrite <- H2. auto. Qed.
(** [] *)
