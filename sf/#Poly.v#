...
Theorem app_nil_r : forall (X:Type), forall l:list X,
  l ++ [] = l.
Proof.
  intros. induction l as []. - auto. - simpl. rewrite -> IHl. auto.
Qed.
...
Proof.
  intros. induction l1 as []. - auto. - simpl. rewrite -> IHl1. auto.
Qed.
(** [] *)
...
Proof.
  ...
  - auto.
  - simpl. SearchAbout (rev (?x ++ ?y)). rewrite -> rev_app_distr. rewrite -> IHl. simpl. auto.
Qed.
(** [] *)
...
(*Take the first elements out of a list of pairs*)
Fixpoint fstl {X Y : Type} (l : list (X*Y)) : (list X) := match l with
  | [] => []
  | hd :: tl => fst hd :: (fstl tl)
end.
Example test_fstl: fstl [(1,false);(2,false)] = [1;2].
Proof. auto. Qed.
(*Take the second elements out of a list of pairs*)
Fixpoint sndl {X Y : Type} (l : list (X*Y)) : (list Y) := match l with
  | [] => []
  | hd :: tl => snd hd :: (sndl tl)
end.
Example test_sndl: sndl [(1,false);(2,false)] = [false;false].
Proof. auto. Qed.

Definition split {X Y : Type} (l : list (X*Y))
               : (list X) * (list Y) := (fstl l, sndl l).
...
(** Use [filter] to write a Coq function [partition]:
   ...
   list. *)
(*Reverse the effect of a X -> bool function*)
Definition anti_func {X : Type} (func : X -> bool) (x : X) : bool := negb (func x).
Example test_anti_fun : anti_func evenb 5 = true.
Proof. auto. Qed.
...
Definition partition {X : Type}
                     ...
                     (l : list X)
                   : list X * list X :=
  (filter test l, filter (anti_func test) l).
...
Proof.
  intros. induction l as [].
  - auto.
  - simpl. rewrite -> map_app. rewrite -> IHl. auto.
Qed.
(** [] *)
...
(* Yeah, in fold_example3, you can just input raw numberes and still returns a list *)
(** [] *)
...
Lemma fold_length_hd_tl : forall (X : Type) (hd : X) (tl : list X), 
  fold_length (hd :: tl) = S (fold_length tl).
Proof.
  intros. auto.
Qed.
...
Proof.
  ...
  - rewrite -> fold_length_hd_tl.  rewrite -> IHl. auto.
Qed.
...
Definition fold_map {X Y:Type} (f : X -> Y) (l : list X) : list Y :=
  fold (fun x => app [(f x)]) l []. (*WTF did I just write?*)

Example fold_map_test1 : fold_map (fun x : nat => 3 + x) [2; 0; 2] = [5; 3; 5].
Proof. auto. Qed.

Example fold_map_test2 : fold_map oddb [2; 1; 2; 5] = [false; true; false; true].
Proof. auto. Qed.

(** Write down a theorem [fold_map_correct] in Coq stating that
   [fold_map] is correct, and prove it. *)
Lemma fold_map_x_l : forall (X Y: Type) (f : X -> Y) (x : X) (l : list X),
...
Proof.
  intros. induction l as [].
  - auto. - rewrite -> fold_map_x_l. simpl. rewrite -> IHl. auto.
Qed.
...
Proof.
  intros. destruct p as [x y]. auto. Qed.
(** [] *)
...
(** Recall the definition of the [nth_error] function:
   ...
   CHALLENGE ACCEPTED!
   Let's go by induction:
   + Se all know that there is no deal for the first case: []
   + Having the hypothesis that nth_error l' length(l') n will return 'None' (With n being length (l')) 
     we must show that nth_error (x : l') n' = None. (with n' being... you know, n+1)
     * To be fair, we do know what would happen from this point, but I'll do this
       nth_error (x : l') n' = nth_error l' (n' - 1) = nth_error l' n = None. Yay, Qed Bitch!
...
Module Church.
(*Just a bit of sugar for eye measure: operation on type X*)
Definition op (X : Type) : Type :=  (X -> X).
Definition nat := forall X : Type, (op X) -> X -> X.

(** Let's see how to write some numbers with this notation. Iterating
    a function once should be the same as just applying it.  Thus: *)

Definition one : nat :=
  fun (X : Type) (f : op X) (x : X) => f x.

(** Similarly, [two] should apply [f] twice to its argument: *)

Definition two : nat :=
  fun (X : Type) (f : op X) (x : X) => f (f x).
...
Definition succ (n : nat) : nat :=
  fun (X : Type) (f : op X) (x : X) => f (n X f x).
...
Definition plus (n m : nat) : nat := (*m nat succ n*)
  fun (X : Type) (f : op X) (x : X) => n X f (m X f x).
...
(** Multiplication: *)
(*Do "... + m" n times to zero*)
Definition mult (n m : nat) : nat :=
  fun (X : Type) (f : op X) (x : X) => n X (m X f) x.
...
Definition exp (n m : nat) : nat := 
  fun (X : Type) (f : op X) (x : X) => fst (m (prod (X -> X) X) (fun (p : prod (X -> X) X) => ((n X (fst p)), n X (fst p) (snd p))) (f, x)) x.
Definition exp_factory (n m : nat) : nat := 
  fun (X : Type) (f : op X) (x : X) => m (X -> X) (fun (repeat_m : op X) => n X repeat_m) f x.

(*
Let me a explain a bit about what is going on here, in case you're going insane
The function works on the product of (X -> X) and X. Why? Because I want to preserve the
function for future use in the loop. as you can see, each time through the loop, the function changes:
+ Initial time (not doing anything, or "power zero"): f x
+ Square : N X f x
+ Cube: N X (N X f) x
And so on... for m times
Each time through the loop, the function gains "N X", and 'x' is x

The function itself should be documented separately:
fun (p : prod (X -> X) X) => ((n X (fst p)), n X (fst p) x)
The input is both the function and the previous result, you can see
separately the output encapsulated in the pair here:
+ function: n X [last function]

The 'fst' in the beginning indicates that we only wants to return the answer
(the 'X' part), because that's what she said. We do that by apply the funciton
to the 'x'.

"Why am I doing it like this?" You ask, because of universe inconsistency,
a "loophole patch" in prolog that prevents me from nat to using itself as 'X'.
That's why you have to act on the function. It's long, but it can be done.

-------UPDATE: FACTORY EXPANSION----------------
This version is even better because it doesn't need to use the stupid pairs, just build the function from f
to f^N^N... and then finally apply the thing to x. I don't know why I can't sleep...
*)
...
Definition seven := succ six.
Definition eight := succ seven.
Definition nine := succ eight.
Definition ten := succ nine.
Example exp_4 : exp zero zero = one.
Proof. reflexivity. Qed.

Example exp_5 : exp two three = eight.
Proof. reflexivity. Qed. 

Example exp_1 : exp three zero = one.
Proof. reflexivity. Qed.
...
Example exp_1' : exp_factory three zero = one.
Proof. reflexivity. Qed.
Example exp_2' : exp_factory two two = four.
Proof. reflexivity. Qed.
Example exp_3' : exp_factory three two = nine.
Proof. reflexivity. Qed.
Example exp_4' : exp_factory zero zero = one.
Proof. reflexivity. Qed.
Example exp_5' : exp_factory two three = eight.
Proof. reflexivity. Qed.
