...
Lemma t_update_eq : forall A (m: total_map A) x v,
  (t_update m x v) x = v.
Proof. unfold t_update. intros. rewrite <- beq_id_refl. auto. Qed.
...
Proof. intros. unfold t_update. apply false_beq_id in H. rewrite H. auto. Qed.
(** [] *)
...
unfold t_update. destruct (beq_id x x0); auto. Qed.
...
Proof. intros. destruct (beq_id x y) eqn:Hb.
- apply beq_id_true_iff in Hb. apply ReflectT. auto.
- apply beq_id_false_iff in Hb. apply ReflectF. auto. Qed.
...
Theorem t_update_same : forall X x (m : total_map X),
  t_update m x (m x) = m.
Proof. intros. apply functional_extensionality. intros.
unfold t_update. destruct (beq_idP x x0). - subst. auto. - auto. Qed.
...
intros. unfold t_update. destruct (beq_idP x1 x), (beq_idP x2 x).
- subst. elim H. auto. - auto. - auto. - auto. Qed.
