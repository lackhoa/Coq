...
Proof. intros n. reflexivity.  Qed. Print mult_0_l.
...
Theorem plus_id_exercise : forall n m o : nat,
  n = m -> m = o -> n + m = m + o.
Proof. intros. rewrite H. rewrite H0. reflexivity. Qed. Print plus_id_exercise.
...
Theorem plus_1_neq_0 : forall n : nat,
  beq_nat (n + 1) 0 = false.
Proof.
intros; destruct n as [| n']; auto. Qed.
...
Inductive bin : Type :=
  | z : bin (*zero*)
  | t : bin -> bin (*twice a binary*)
  | st : bin -> bin. (*one more than twice a binary*)

Fixpoint incr (b : bin) : bin := match b with
  | z => st z
  | t b' => st b'
  | st b' => t (incr b') 
end.
  
Fixpoint bin_to_nat (b: bin) : nat := match b with
  | z => 0
  | t b' => (bin_to_nat b') * 2
  | st b' => S ((bin_to_nat b') * 2)
end.

(*Testing incr*)
Compute (incr z). Compute (incr (t (st z))).
Compute (incr (st (t (st z)))).

(*Testing bin_to_nat*)
Compute (bin_to_nat (incr z)). Compute (bin_to_nat (t (st z))).
Compute (bin_to_nat (incr (st (t (st z))))).
