...
Theorem rev_exercise1 : forall (l l' : list nat),
     ...
     l' = rev l.
Proof.
  intros. rewrite -> H. rewrite -> rev_involutive. auto.
Qed.
(** [] *)
...
Rewrite can be used to replace any terms with another, but it can't be used if the hypothesis
has an implication that must be satisfied.
When can both be applied? When the hypothesis matches the goal exatly
...
Example trans_eq_exercise : forall (n m o p : nat),
     ...
     (n + p) = (minustwo o).
Proof.
  intros. apply trans_eq with (m := m) ; auto.
Qed.
(** [] *)
...
Example inversion_ex3 : forall (X : Type) (x y z : X) (l j : list X),
  ...
  x = y.
Proof.
  intros. inversion H. inversion H0. inversion H2 as [H100]. auto.
Qed.
(** [] *)
...
Proof.
  intros n eq H.
  symmetry in H. (* symmetry. apply eq. assumption.*)
...
Theorem beq_nat_true_failed : forall n m,
...
Proof.
  ...
  - intros. destruct m as [| m'].
    + auto. + inversion H.
  - intros. destruct m as [| m'].
    + inversion H. + apply f_equal. apply IHn'. inversion H. auto.
Qed.
(** [] *)
...
(* COUNT ME IN!*)
(*
We need to show that: for all natural numbers n and m, "beq_nat n m = true" implies 'n =m'.
+ Suppose we have a natural number n, we'll show that for all m, 'beq_nat n m = true' implies
'n = m'
+ Using induction on n:
  - First case : n = 0. We know that 'beq_nat m 0 = true' is equivalent to m = 0. which is m = n
  - Second case : n = S n'. The induction hypothesis is: "For all m, beq_nat m n' = true' implies 
m = n'. We need to show that "for all m, beq_nat m S n' = true" implies m = S n'
Let's introduce m in two ways: 0 and S m'.
    | First case: m = 0. But since we know that  beq_nat m S n' is true, we can count this out
because of Contradiction
    | second case: m = S m'. We do have beq_nat (S m') (S n') = true from the beginning hypothesis
. And the induction hypothesis gave us "forall m, beq_nat n' m =true ' implies n' = m
 We have to show that: S n' = S m'. We can deduce easily beq_nat m' n' = true. And the goal can
be reduced to n' = m'. Yeah, now we just have to apply IHn' and we're done. Oh my god, why is
this thing so hard?
*)
(** [] *)
...
Lemma nth_error_cons : forall (X : Type) (x : X) l n,
  nth_error l n = nth_error (x :: l) (S n).
Proof.
  intros. auto.
Qed.
...
Theorem nth_error_after_last: forall (n : nat) (X : Type) (l : list X),
     ...
     nth_error l n = None.
Proof.
  intros n X l eq. generalize dependent n. (*Put n back*) induction l as [| hd l'].
  + auto.
  + intros. assert (hd :: l' = [hd] ++ l').
    - auto.
    - rewrite -> H in eq. clear H. rewrite -> app_length in eq. simpl in eq. destruct n as [| n'].
      * inversion eq.
      * inversion eq. apply IHl' in H0. simpl. auto.
Qed.
(** [] *)
...
Proof.
  (*INDUCTION ON l*)
  intros X Y l. unfold split. induction l as [| xy l'].
  + intros. inversion H. auto.
  + intros. simpl in H. inversion H. simpl.
  assert ((fst xy, snd xy) = xy). { destruct xy as []. auto. }
  rewrite -> H0. clear H0. destruct l1 as [| hx l1'] .
    { inversion H1. }
   destruct l2 as [| hy l2'].
   { inversion H2. }
   inversion H1. inversion H2. clear H H1 H3 H5.
  (*The final blow*) assert ( combine (fstl l') (sndl l') = l' ).
    - apply IHl'. auto.
    - rewrite -> H. auto.
Qed.
(** [] *)
...
Proof.
  ...
  + destruct (f true) as [] eqn:Hf_true.
    - rewrite -> Hf_true. rewrite -> Hf_true. auto.
    - destruct (f false) as [] eqn:Hf_false. * apply Hf_true. * apply Hf_false.
  + destruct (f false) as [] eqn:Hf_false.
    - destruct (f true) as [] eqn:Hf_true. * assumption. * assumption.
    - rewrite -> Hf_false. assumption.
Qed.
(** [] *)
...
Proof.
  intros. destruct (beq_nat n m) as [] eqn:H_n_m.
  + apply beq_nat_true in H_n_m. rewrite -> H_n_m. apply beq_nat_refl.
  + generalize dependent m. induction n as [| n'].
    - intros. destruct m as [| m'].
      * rewrite <- beq_nat_refl in H_n_m. inversion H_n_m.
      * simpl. auto.
    - intros. destruct m as [| m'].
      * auto.
      * simpl in H_n_m. simpl. apply IHn'. assumption.
Qed.
(** [] *)
...
Theorem beq_nat_trans : forall n m p,
  ...
  beq_nat n p = true.
Proof.
  intros n m p Heq_nm Heq_mp.
  assert (n = m). { apply beq_nat_true. auto. } clear Heq_nm.
  assert (m = p). { apply beq_nat_true. auto. } clear Heq_mp.
  assert (n = p). { apply trans_eq with m; auto. }
  rewrite -> H1. rewrite <- beq_nat_refl. auto.
Qed.
(** [] *)
...
Proof.
  ...
  + intros. unfold combine in H0. destruct l2 as [| hd2 l2'].
    - rewrite <- H0. auto. - inversion H.
  + intros. destruct l2 as [| hd2 l2'].
    - inversion H.
    - simpl in H0. simpl in H. inversion H. clear H.
    destruct l as [| xy l']. { inversion H0. }
    inversion H0.
    assert (split l' = (l1' , l2')). { apply IHl1' ; auto. }
     rewrite -> H3. unfold split. simpl. unfold split in H. inversion H.
     auto.
Qed.
(** [] *)
...
Proof.
  ...
  induction l as [| hd l']. { intros. inversion H. }
  intros. simpl in H. destruct (test hd) as [] eqn:Htest_hd.
...
Fixpoint forallb { X : Type } (test : X -> bool) (l : list X) := match l with
  | nil => true
  | hd :: tl => (andb (test hd) (forallb test tl))
end.

Example test_forallb_1 : forallb oddb [1;3;5;7;9] = true. Proof. auto. Qed.
Example test_forallb_2 : forallb negb [false;false] = true. Proof. auto. Qed.
Example test_forallb_3 : forallb evenb [0;2;4;5] = false. Proof. auto. Qed.
Example test_forallb_4 : forallb (beq_nat 5) [] = true. Proof. auto. Qed.

Fixpoint existsb { X : Type } (test : X -> bool) (l : list X) := match l with
  | nil => false
  | hd :: tl => if (test hd) then true else existsb test tl
end.

Example test_existb_1 : existsb (beq_nat 5) [0;2;3;6] = false. Proof. auto. Qed.
Example test_existb_2 : existsb (andb true) [true;true;false] = true. Proof. auto. Qed.
Example test_existb_3 : existsb oddb [1;0;0;0;0;3] = true. Proof. auto. Qed.
Example test_existb_4 : existsb evenb [] = false. Proof. auto. Qed.

Definition existsb' { X : Type } (test : X -> bool) (l : list X) :=
  negb (forallb (anti_func test) l).

Example test_existb'_1 : existsb' (beq_nat 5) [0;2;3;6] = false. Proof. auto. Qed.
Example test_existb'_2 : existsb' (andb true) [true;true;false] = true. Proof. auto. Qed.
Example test_existb'_3 : existsb' oddb [1;0;0;0;0;3] = true. Proof. auto. Qed.
Example test_existb'_4 : existsb' evenb [] = false. Proof. auto. Qed.

Theorem existsb_existsb' : forall (X : Type) (test : X -> bool) (l : list X),
  existsb test l = existsb' test l.
Proof.
  intros. induction l as [| x l']. { auto. }
  (*Simplifying the goal*)simpl. unfold existsb'. simpl. unfold anti_func.
  (*Simplifying IHl'*)unfold existsb' in IHl'. unfold anti_func in IHl'.
  destruct (test x) as [] eqn:Htest_x. { simpl. auto. }
