...
Proof.
  intros. split. + destruct n as [| n']. - auto. - simpl in H. inversion H. + destruct n as [| n']. - simpl in H.
...
Proof.
  ...
  + left. auto. + destruct m as [| m']. - right. auto. - simpl in H. inversion H.
Qed.
(** [] *)
...
(*My version*)
Fact our_not_implies_not : forall (P : Prop),
  P /\ (forall (Q:Prop), P -> Q) -> ~ P.
Proof.
  intros. destruct H. apply H0. auto. (*Oh my god P went back in time and destroyed itself.*)
Qed.
(** [] *)
...
(*CHALLENGE ACCEPTED!
Given a property P, prove not not P.
Alrighty, first we'll expand the definition of 'not', we should get:
(P -> False) -> False.
Since we have P, we can get False. And that's Qed. What else do you want?
*)
(** [] *)
...
Theorem contrapositive : forall (P Q : Prop),
  (P -> Q) -> (~Q -> ~P).
Proof.
  intros. unfold not. intros. apply H0. apply H. apply H1.
Qed.
(** [] *)
...
Theorem not_both_true_and_false : forall P : Prop,
  ~ (P /\ ~P).
Proof.
  intros. unfold not. intros. destruct H. auto.
Qed.
(** [] *)
...
Theorem iff_refl : forall P : Prop,
  P <-> P.
Proof.
  intros. split. + intros. auto. +  intros. auto.
Qed. 

Theorem iff_trans : forall P Q R : Prop,
  (P <-> Q) -> (Q <-> R) -> (P <-> R).
Proof.
  intros. destruct H as [HAB HBA]. destruct H0 as [HBC HCB].
  split. + auto. + auto.
Qed.
(** [] *)
...
Proof.
  intros. split.
  + intros. destruct H. - split; auto. - destruct H. split; auto.
  + intros. destruct H. destruct H. - auto. - destruct H0; auto.
Qed.  
(** [] *)
...
Proof.
  unfold not. intros. elim H0. intros. apply H1. apply H.
Qed.
(** [] *)
...
Proof.
  ...
  + exists x. left; apply H. + exists x; right; apply H.
Qed.
(** [] *)
...
Theorem and_distributes_over_or : forall P Q R, P /\ (Q \/ R) <-> (P /\ Q) \/ (P /\ R).
Proof.
  firstorder.
Qed.
...
Proof.
  split; intros.
  + induction l as [| a l'].
    - inversion H. - simpl. simpl in H. destruct H.
      * exists a. auto.
      * destruct IHl'.
        { auto. } { exists x. split. apply H0. right. apply H0. }
  + induction l as [| a l'].
...
Proof.
  ...
  + induction l as [| hd tl].
    ...
    - simpl in H. destruct H.
      ...
      *apply IHtl in H. simpl. destruct H. { left. right. auto. } { right. auto. }
  + destruct H. induction l as [| hd tl].
    - inversion H.
    - simpl. simpl in H. destruct H.
      * left. auto.
      * apply IHtl in H. right. auto.
    - induction l as [| hd tl].
      * simpl. auto.
      * simpl. right. auto.
Qed.
...
Fixpoint All {T : Type} (P : T -> Prop) (l : list T) : Prop := match l with
  | nil => True
  | hd :: tl => P hd /\ All P tl
end.

Theorem or_imply : forall (P Q R : Prop), ((P \/ Q) -> R)
  <-> ((P -> R) /\ (Q -> R)).
Proof.
  intros. split.
  + split. - intros. apply H. left. auto. - intros. auto.
  + intros. destruct H. destruct H0. - auto. - auto.
Qed.
...
Proof.
  ...
  + unfold combine_odd_even. simpl. unfold oddb in H0. auto.
  + unfold combine_odd_even. destruct (oddb (S n')) as [].
    - apply H. auto.
    - apply H0. auto.
Qed.
...
Theorem combine_odd_even_elim_odd :
  forall (Podd Peven : nat -> Prop) (n : nat),
    ...
    Podd n.
Proof.
  intros. unfold combine_odd_even in H. rewrite -> H0 in H. auto.
Qed.
...
Proof.
  intros. generalize dependent l2. induction l1 as [| x1 tl1].
  + intros; auto.
  + intros. simpl. rewrite -> IHtl1. rewrite -> IHtl1.
  rewrite -> app_nil_r. rewrite -> (IHtl1 [x1]). rewrite <- app_assoc.
  simpl. auto.
Qed.

Lemma tr_rev_app : forall (X : Type) (l1 l2 : list X),
  tr_rev (l1 ++ l2) = tr_rev l2 ++ tr_rev l1.
Proof.
  intros. unfold tr_rev. simpl. generalize dependent l2. induction l1 as [| hd1 tl1].
  + simpl. intros. rewrite -> app_nil_r. auto.
  + simpl. intros. rewrite -> (rev_append_app _ tl1 [hd1]). 
  rewrite -> app_assoc. rewrite <- IHtl1. rewrite <- rev_append_app.
  auto.
Qed.
...
Proof.  
  ...
  induction x as [| x' tl].
  + auto.
  + simpl. unfold tr_rev. simpl. unfold tr_rev in IHtl. simpl in IHtl.
  destruct tl as [| hd tl'].
    - simpl. auto.
    - simpl. simpl in IHtl. rewrite <- IHtl. rewrite -> (rev_append_app).
    rewrite -> (rev_append_app X tl' [hd]). rewrite <- app_assoc.
    simpl. auto.
Qed.   
...
Proof.
  ...
  induction n as [| n'].
  + simpl. exists 0. auto.
  + rewrite -> evenb_S. destruct (evenb n') as [| ] eqn:Hevenbn'.
    - simpl. SearchAbout (_ = _). destruct IHn'. exists x. auto.
    - destruct IHn'. simpl. rewrite -> H. exists (S x). auto.
Qed. 
(** [] *)
...
Proof.
  intros. split; intros.
  + split.
    - apply (andb_true_elim2 b2). rewrite -> andb_commutative. auto.
    - apply (andb_true_elim2 b1). auto.
...
Proof.
  intros. split; intros.
  + destruct b1, b2 as [].
    - left. auto. - left. auto. - right; auto. - inversion H.
  + destruct H; rewrite -> H. - auto. - destruct b1 as []. * auto. * auto.
Qed.
...
Proof.
  split.
  + intros. generalize dependent y. induction x as [| x']; intros.
    - destruct y as [| y']. * inversion H. * auto.
    - destruct y as [| y']. * auto. * simpl in H. unfold not. simpl. intros.
    inversion H0. apply IHx' in H. elim H. apply H2.
  + intros. generalize dependent y. induction x as [| x']; intros; destruct y as [| y'].
    - elim H. auto. - simpl. auto.
    -  simpl. auto. - simpl. unfold not in H. unfold not in IHx'.
    apply IHx'. intros. elim H. apply f_equal. auto.
Qed.
...
Fixpoint beq_list {A : Type} (beq : A -> A -> bool)
                  (l1 l2 : list A) : bool := match l1 with
  | nil => match l2 with
    | nil => true
    | _ :: _ => false
    end
  | hd1 :: tl1 => match l2 with
    | nil => false
    | hd2 :: tl2 => (beq hd1 hd2) && (beq_list beq tl1 tl2)
    end
  end.

Example beq_list_ex1 : beq_list beq_nat [] [] = true. Proof. auto. Qed.
Example beq_list_ex2 : beq_list beq_nat [] [2; 6] = false. Proof. auto. Qed.
Example beq_list_ex3 : beq_list beq_nat [4; 7] [] = false. Proof. auto. Qed.
Example beq_list_ex4 : beq_list beq_nat [1; 3; 4; 6] [1; 3; 4; 6] = true. Proof. auto. Qed.
...
Proof.
  intros. generalize dependent l2. induction l1 as [| hd1 tl1]; intros.
  + split.
    - destruct l2 as [| hd2 tl2]. * simpl. auto. * intros. inversion H0.
    - intros. rewrite <- H0. auto.
  + split.
    - intros. destruct l2 as [| hd2 tl2]. * inversion H0.
      * simpl in H0. rewrite -> andb_true_iff in H0. destruct H0.
      apply H in H0. rewrite <- H0.
      apply IHtl1 in H1. rewrite <- H1.
      auto.
    - intros. rewrite <- H0. simpl. rewrite -> andb_true_iff. split.
      * apply H. auto.
      * apply IHtl1. auto.
Qed.
...
Proof.
  split.
  + induction l as [| hd l']; intros.
    - simpl. auto.
    - simpl. simpl in H. apply andb_true_iff in H. destruct H. split. * auto. * auto.
  + induction l as [| hd l']. intros.
    - simpl. auto.
    - simpl. intros. destruct H. rewrite -> H. simpl. apply IHl' in H0. auto.
Qed.
...
Theorem excluded_middle_irrefutable:  forall (P:Prop),
  ~ ~ (P \/ ~ P).
Proof.
  unfold not. intros. rewrite -> or_imply in H . destruct H.
  apply H0. apply H.
Qed.
...
Theorem excluded_middle_irrefutable':  forall (P:Prop),
  ~ ~ (P \/ ~ P).
Proof.
  unfold not. intros. apply H. right. intro. apply H . left. apply H0. 
Qed.
...
Proof.
  ...
  destruct H with (P x). - auto. - exfalso. apply H0. exists x. auto.
Qed.
(** [] *)
...
Theorem lem_peirce : excluded_middle <-> peirce.
Proof.
  unfold excluded_middle. unfold peirce. unfold not. split; intros.
  + destruct H with Q. - auto. - destruct H with P.
  * auto. * apply H0. intro. elim H2. auto. (*Literally breaking down everything*)
  + apply H with (P -> False). intro. right. intro. apply H0 in H1. *  auto. * left. auto.
Qed.

Theorem lem_double_neg : excluded_middle <-> double_negation_elimination.
Proof.
  unfold excluded_middle. unfold double_negation_elimination. unfold not. split; intros.
  + destruct H with P. - auto. - exfalso. apply H0 in H1. auto.
  + apply H. intro. apply H0. right. intro. apply H0. left; auto.
Qed.

Theorem lem_not_and_not : excluded_middle <-> de_morgan_not_and_not.
Proof.
  unfold de_morgan_not_and_not. unfold excluded_middle. unfold not.
  split; intros.
  + destruct H with P. - auto. - destruct H with Q. * auto.
  * exfalso. apply H0. split; auto.
  + apply H. intro. destruct H0. apply H1 in H0. auto.
Qed.

Theorem lem_implies_to_or : excluded_middle <-> implies_to_or.
Proof.
  unfold excluded_middle. unfold implies_to_or. unfold not.
  split; intros.
  + destruct H with P. - auto. - auto.
  + apply or_comm. apply H. intro. auto.
Qed.
