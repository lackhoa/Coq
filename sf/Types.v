...
(*value encapsulates boolean and numeric*)
...
Example some_term_is_stuck :
  exists t, stuck t.
Proof. exists (tsucc ttrue). unfold stuck. split.
- intro. destruct H. inversion H; subst. inversion H1. (*Stuck at ttrue*)
- unfold value. intro. destruct H.
  + inversion H. + inversion H; subst. inversion H1. Qed.
...
Proof. intros. intro. induction t.
- inversion H0. inversion H1.
- inversion H0.  inversion H1.
- inversion H0; subst. inversion H. inversion H2.
inversion H2.
- inversion H0. inversion H1.
- inversion H0; clear H0. inversion H1; subst.
inversion H; subst. inversion H0. inversion H0; subst.
apply IHt; try assumption. unfold value. right; assumption.
exists t1'. assumption.
- inversion H0; clear H0. inversion H; subst. inversion H0.
inversion H0.
- inversion H0; clear H0. inversion H. inversion H0.
inversion H0. Qed.
...
Theorem step_deterministic:
  deterministic step.
Proof with eauto. unfold deterministic.

induction x; intros; subst; eauto; try congruence.
- inversion H. - inversion H.
- inversion H; inversion H0; try congruence; eauto; subst.
  + inversion H9. + inversion H9. + inversion H5.
  + inversion H5. + assert (t1' = t1'0); eauto. subst; reflexivity.
- inversion H.
- inversion H; subst. inversion H0; subst.
assert (t1' = t1'0); eauto. subst; reflexivity.
- inversion H; inversion H0; subst; eauto.
  + inversion H3.
  + inversion H4.
  + inversion H5.
  + subst. inversion H4. reflexivity.
  + exfalso. eapply value_is_nf. unfold value; right.
  eapply nv_succ. eassumption. exists t1'; assumption.
  + inversion H2.
  + exfalso. eapply value_is_nf. unfold value; right.
  eapply nv_succ. eassumption. exists t1'; assumption.
  + assert (t1' = t1'0). { eapply IHx; assumption. }
  subst; reflexivity.
(*That part made me cry, internally, I forgot about 
explosion theorem for no good reason, and it 
taught me, HARD*)
- inversion H; inversion H0; subst; try reflexivity.
  ...
  eapply nv_succ. eassumption. exists t1'; assumption.
  + apply f_equal. eauto.
Qed. (*Was  that it? Is it finally over?*)
...
- destruct IHHT.
  ...
  apply (nat_canonical t1 HT) in H. apply nv_succ...
  + right. destruct H. exists (tsucc x). auto.
- destruct IHHT.
  + unfold value. apply (nat_canonical t1 HT) in H.
  right. destruct t1; inversion H. eauto. eauto.
  + destruct H. right. eauto.
- destruct IHHT.
  + apply (nat_canonical t1 HT) in H. right.
  destruct t1; inversion H. * eauto. * eauto.
...
eauto.
- inversion HE; subst.
  + eauto. + inversion HT. assumption.
  + apply IHHT in H0. eauto.
...
Theorem preservation' : forall t t' T,
  ...
  t ==> t' ->
  |- t' \in T.
Proof with eauto. intros t t' T HT HE.
generalize dependent T.
induction HE; intros; inversion HT; subst; eauto.
inversion H1; subst. assumption. Qed.
...
Theorem subject_expansion : exists t t' T,
  ...
  |- t' \in T ->
  ~ (|- t \in T).
Proof. exists (tif ttrue tfalse tzero), tfalse, TBool.
intros. intro. inversion H1; subst. inversion H8. Qed.
