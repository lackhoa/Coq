...
Theorem plus_one_r' : forall n:nat,
  n + 1 = S n.
Proof. apply nat_ind. - auto. - intros. simpl. auto. Qed.
(** [] *)
...
Inductive khoa_natlist : Type :=
  | khoa_nil : khoa_natlist
  | khoa_cons : nat -> nat -> khoa_natlist -> khoa_natlist.
Check khoa_natlist_ind.
(** Now what will the induction principle look like? *)
(*IT CHANGES QUITE A BIT ABOUT HOW TO REASON
COQ WILL KEEP THE RIGHT-MOST VALUE AND VARY OTHER ARGUMENTS
WRONG! IT DOESN'T CHANGE AT ALL ABOUT WHAT IT MEANS*)
(** [] *)
...
(*byntree_ind : forall P : byntree -> Prop := 
  ...
  (forall (b : byntree), P b -> forall (y : yesno) (b1 : byntree), P (nbranch y b1 b)) ->
  forall (b: byntree) ->  P byntree.
*) (*WRONG: I LACKED P B1*)
Check byntree_ind.
(** [] *)
...
(*tree_ind : forall (X: Type) (P: tree -> Prop),
...
Check foo_ind. (*WHAT? HOW DOES IT KNOW?*)
(** [] *)
...
Theorem plus_assoc'' : forall n m p, P_plus_assoc'' n m p.
Proof. induction n. - intros. unfold P_plus_assoc''. reflexivity.
- intros. unfold P_plus_assoc''. simpl. unfold P_plus_assoc'' in IHn.
rewrite IHn. auto. Qed.

Definition P_plus_comm'' (n m: nat) := n + m = m + n.
Theorem plus_comm''' : forall n m, P_plus_comm'' n m.
Proof. induction n. - intros. unfold P_plus_comm''. rewrite <- plus_n_O. auto.
- intros. unfold P_plus_comm''. simpl. rewrite <- plus_n_Sm.
unfold P_plus_comm'' in IHn. rewrite IHn. auto. Qed.
(** [] *)
