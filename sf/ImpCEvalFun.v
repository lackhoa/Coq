...
Definition set_Z : com :=
  ...
  ELSE
    Z ::= (ANum 1)
  FI.
Compute (test_ceval (t_update empty_state X 5) set_Z).
Compute (test_ceval (t_update empty_state X 2) set_Z).
...
induction Hce.
- exists 1; auto.
- exists 1. rewrite <- H. auto.
- destruct IHHce1 as [i1 H1]; destruct IHHce2 as [i2 H2].
exists (i1 + i2). simpl. destruct (i1 + i2) eqn:Hi12.
  + assert (i1 = 0) as Hi1. { omega. } rewrite Hi1 in H1. inversion H1.
  + simpl. destruct i2 as [| i2'].
    * inversion H2.
    * assert (i1 + i2' = n) as Hi12'. { omega. } clear Hi12.
    assert (ceval_step st c1 n = Some st') as Hc1.
    { apply ceval_step_more with i1; [omega | auto]. }
    subst. rewrite Hc1. clear Hc1. destruct i1 as [| i1'].
      { inversion H1. }
      { apply ceval_step_more with (S i2'); [omega | auto]. }
- destruct IHHce as [i Hc1]. exists (S i). simpl. rewrite H. assumption.
- destruct IHHce as [i Hc1]. exists (S i); simpl. rewrite H. assumption.
- exists 1. simpl. rewrite H. reflexivity.
- destruct IHHce1 as [i1 IHHce1]; destruct IHHce2 as [i2 IHHce2].
exists (S (i1 + i2)). simpl. rewrite H. 
assert (ceval_step st c (i1 + i2) = Some st') as Hce.
{ apply ceval_step_more with i1. omega. assumption. }
rewrite Hce. apply ceval_step_more with i2. omega. assumption.
Qed.
...
Proof.
  ...
  inversion He2 as [i2 E2]. clear He1 He2.
  apply ceval_step_more with (i2 := i1 + i2) in E1.
  apply ceval_step_more with (i2 := i1 + i2) in E2.
  (*THAT STEP WAS INGENIOUS!*)
