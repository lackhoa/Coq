...
(* as1: X is 3
as2: X is not larger than Y
as3: as1 or as2 is true
as4: Z^2 is no larger than X or (Z+1)^2 is larger than X
as5: always holds
as6: nah, thou shall not pass!
 *)
End ExAssertions.
(** [] *)
...
(** Paraphrase the following Hoare triples in English.
   ...
   1) {{True}} c {{X = 5}}
For every start state, the command terminates with X = 5
   2) {{X = m}} c {{X = m + 5)}}
The end state will add 5 to the start state
   3) {{X <= Y}} c {{Y <= X}}
If the start state has X <= Y, then then end will have Y <= X
   4) {{True}} c {{False}}
Q never holds
   5) {{X = m}}
      c
      {{Y = real_fact m}}    
Y in the end will be set to real_fact of X in the start state
   6) {{True}}
      c
      {{(Z * Z) <= m /\ ~ (((S Z) * (S Z)) <= m)}}
For every start state, the end state has Z^2 <= m and 
(Z+1)^2 > m
*)
...
HOLDS
   9) {{X = 1}}
      WHILE X <> 0 DO X ::= X + 1 END
      {{X = 100}}
HOLDS (IT NEVER TERMINATES SO WHO GIVES A SHIT?!)
*)
(** [] *)
...
Theorem hoare_post_true : forall (P Q : Assertion) c,
  (forall st, Q st) ->
  {{P}} c {{Q}}.
Proof. unfold hoare_triple. intros. apply H. Qed.
...
Proof. intros. rename H into Hfex. split.
- inversion H; subst. rewrite t_update_shadow.
simpl in H0. inversion H0. rewrite <- H2.
rewrite t_update_same. assumption.
- simpl in H0. inversion H; subst. rewrite t_update_shadow.
rewrite t_update_eq. inversion H0.
rewrite <- H2. rewrite t_update_same. reflexivity. Qed.
(** [] *)
...
Theorem hoare_asgn_fwd_exists :
  ...
  {{fun st => exists m, P (t_update st X m) /\
                st X = aeval (t_update st X m) a }}.
Proof. intros functional_extensionality a P. 
unfold hoare_triple. intros. inversion H; subst.
exists (aeval st (AId X)). rewrite t_update_shadow.
rewrite t_update_eq. split; rewrite t_update_same.
...
- apply hoare_asgn. - intro. intros. unfold assn_sub.
simpl. unfold t_update; simpl. assumption. Qed.
...
Proof. intro. intros. inversion H; subst. 
inversion H6; inversion H3; subst. split.
- simpl. unfold t_update; simpl. reflexivity.
- simpl. unfold t_update; simpl. reflexivity. Qed. 
...
Theorem swap_exercise :
  ...
  swap_program
  {{fun st => st Y <= st X}}.
Proof. unfold swap_program. intro. intros. 
inversion H. inversion H2; subst.  clear H7. inversion H6; subst.
clear H6 H. inversion H3; inversion H4; inversion H8; subst.
simpl. unfold t_update. simpl. assumption. Qed.
(** [] *)
...
(* You changed the state of X you moron! How many more times
will you learn? *)
(** [] *)
...
Proof. unfold hoare_triple. intros. inversion H; subst.
-  simpl in H6. inversion H7; subst. simpl. unfold t_update.
simpl. apply leb_complete in H6. omega.
- simpl in H6. inversion H7; subst.
unfold t_update; simpl. reflexivity. Qed.
...
Theorem hoare_if1 : forall P Q b c,
  {{fun st => P st /\ bassn b st}} c {{Q}} -> (*If true, P going through c will get Q*)
  (forall  st,  P st /\ ~(bassn b st) -> Q st) -> (*If false, if P then Q*)
  {{P}} (IF1 b THEN c FI) {{Q}}.
Proof. unfold hoare_triple. intros.
inversion H1; subst.
- eapply H. eassumption. split; try assumption.
- apply H0. split; try assumption. 
apply bexp_eval_false in H7. assumption. Qed.
...
simpl. unfold t_update; simpl. assumption.
- intros. destruct H. unfold bassn in H0.
unfold not in H0. simpl in H0.
destruct (NPeano.Nat.eqb_spec (st Y) 0).
  + rewrite e in H. omega. + exfalso; apply H0. reflexivity. Qed.
End If1.
(** [] *)
...
Inductive ceval : state -> com -> state -> Prop :=
  ...
  | E_WhileLoop : forall st st' st'' b1 c1,
   ...
   | E_RepeatEnd : forall b st st' c,
     ceval st c st' ->
     beval st' b = true -> (*Doesn't care about st, we're doing it anyway*)
...
Proof. constructor. - eapply E_Seq. constructor.
reflexivity. constructor. reflexivity.
- reflexivity. Qed.
...
(*OH MY GOD THIS THING IS A LITERAL PAIN
(AND I MEAN THE ONE BELOW THAT)*)
(*Anyways, this is when P is guaranteed*)
Theorem hoare_repeat_mono : forall P b c,
  {{fun st => P st }} c {{P}} ->
  {{P}} REPEAT c UNTIL b END {{fun st => P st /\ bassn b st}}.
Proof. unfold hoare_triple. intros. 
remember (REPEAT c UNTIL b END) as re eqn:Hre.
induction H0; inversion Hre; subst; clear Hre.
- split. eapply H. eassumption. assumption. assumption.
- apply IHceval2. reflexivity. eapply H. eassumption. assumption. Qed. 

Theorem repeat_term : forall b c st st',
  REPEAT c UNTIL b END / st \\ st' ->
  beval st' b = true.
Proof. intros.
remember (REPEAT c UNTIL b END) as re eqn:Hre.
induction H; inversion Hre; clear Hre; subst.
- clear IHceval. assumption.
- apply IHceval2. reflexivity. Qed.

(*This is when Q is guaranteed after P
(But weaked so that can be applied more easily)*)

Theorem hoare_repeat : forall P Q b c,
  ({{P}} c {{Q}} /\
  {{fun st => Q st /\ ~ (bassn b st)}} c {{Q}}) \/ (*If it re-enters the loop, then it definitely satisfied the condition*)
  ({{P}} c {{fun st => Q st /\ bassn b st}}) -> (*If no, then it doesn't need to preserve Q*)
  {{P}} REPEAT c UNTIL b END {{fun (st: state) => Q st /\ bassn b st}}.
Proof. unfold hoare_triple. intros.
destruct H.
- destruct H. inversion H0; subst.
  + apply H with st st' in H1. split;
  assumption. assumption.
  + apply H with st st'0 in H1. Focus 2. assumption.
  split.
  assert (forall Q b c,
  {{fun st => Q st /\ ~ (bassn b st)}} c {{Q}} ->
  {{fun st => Q st /\ ~ (bassn b st)}} (REPEAT c UNTIL b END) {{Q}}) as Hsec.
  { unfold hoare_triple. intros. 
  remember (REPEAT c0 UNTIL b0 END) as re eqn:Hre.
  induction H4; inversion Hre; clear Hre; subst.
    - eapply H3. eassumption. assumption.
    - eapply IHceval2. reflexivity. split.
      + eapply H3. eassumption. assumption.
      + unfold bassn. intro. rewrite H4 in H8. inversion H8. }
    unfold hoare_triple in Hsec. eapply Hsec.
      eassumption. eassumption. split. assumption.
      intro. unfold bassn in H3. rewrite H5 in H3.
      inversion H3.
    eapply repeat_term. eassumption.
(*THAT WAS TOUGH*)
- inversion H0; subst.
  + split. eapply H. eassumption. assumption. assumption.
  + apply H in H6. Focus 2. assumption. destruct H6.
  unfold bassn in H3. rewrite H3 in H4. inversion H4.
Qed. (*This is like playing ZvP*)
...
eapply hoare_consequence_post with 
  (Q' := (fun (st: state) => (st Y) > 0 /\ bassn (BEq (AId X) (ANum 0)) st)).
Focus 2. intro. intros. firstorder. inversion H0.
apply beq_nat_true_iff in H2. assumption.
apply hoare_repeat. left.
unfold hoare_triple. split.
- intros. inversion H; subst. inversion H4; inversion H6; subst.
unfold t_update; simpl. assumption.
- intros. inversion H; subst. inversion H4; inversion H6; subst.
unfold t_update; simpl. destruct H0. unfold bassn in H1.
simpl in H1. unfold not in H1. clear H6; clear H4. clear H.

destruct (NPeano.Nat.eqb_spec (st X) 0). exfalso; apply H1; reflexivity.
omega. Qed.
(*TAKE THAT!*)
...
Definition havoc_pre (X : id) (Q : Assertion) : Assertion :=
  fun (st: state) => (forall (n: nat), Q [X |-> (ANum n)] st).

Theorem hoare_havoc : forall (Q : Assertion) (X : id),
  {{ havoc_pre X Q }} HAVOC X {{ Q }}.
Proof. unfold havoc_pre, hoare_triple, assn_sub. simpl.
