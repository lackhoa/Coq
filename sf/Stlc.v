...
Fixpoint subst (x:id) (s:tm) (t:tm) : tm :=
  ...
  | tabs x' T t1 =>
      tabs x' T (if beq_id x x' then t1 else ([x:=s] t1)) (*x is bound by abstraction*)
  | tapp t1 t2 =>
      tapp ([x:=s] t1) ([x:=s] t2)
  | ttrue =>
      ttrue
...
Inductive substi (s:tm) (x:id) : tm -> tm -> Prop :=
  | s_var1 (*Substitution*) : 
      substi s x (tvar x) s
  |s_var2 (*Different*) : forall x':id, 
    x' <> x ->
    substi s x (tvar x') (tvar x')
  | s_app (*Application*) : forall (t1 t2 t1' t2': tm),
    substi s x t1 t1' ->
    substi s x t2 t2' ->
...
Proof. split.
- generalize dependent t'. induction t; simpl; intros.
  + destruct (beq_id x0 i) eqn:Heq.
    * subst. apply beq_id_true_iff in Heq. subst. auto.
    * subst. apply beq_id_false_iff in Heq. auto.
  + rewrite <- H. auto.
  + rewrite <- H. destruct (beq_id x0 i) eqn: Heq.
      * apply beq_id_true_iff in Heq. subst. debug auto.
...
- generalize dependent t'. induction t; intros;
  ...
  + destruct (beq_id x0 i) eqn:Heq; auto.
  unfold not in H1. apply beq_id_true_iff in Heq.
  symmetry in Heq. apply H1 in Heq; inversion Heq.
  + apply IHt1 in H2. subst. apply IHt2 in H4. subst.
  reflexivity.
  + rewrite <- beq_id_refl. reflexivity.
  + destruct (beq_id x0 i) eqn:Heq.
    * apply beq_id_true_iff in Heq. unfold not in H4;
    apply H4 in Heq. inversion Heq.
    * apply IHt in H5. subst. auto.
  + apply IHt1 in H3. apply IHt2 in H5.
  apply IHt3 in H6. subst. auto.
Qed.
...
Proof.
  eapply multi_step.
    ...
    apply v_abs.
  simpl.
  apply multi_refl. Qed. (*Weird, is that a new way to apply reasoning
step-by-step?*)
...
apply ST_AppAbs. constructor. simpl. eapply multi_step.
apply ST_AppAbs. constructor. simpl. eapply multi_refl. Qed.
