...
Proof.
  ...
  apply hoare_while.
  - (* Loop body preserves invariant WITH GUARD *)
...
Definition slow_ass := forall (m: nat),
  {{ fun st => st X = m }}
      Y ::= ANum 0;;
      WHILE (BNot (BEq (AId X) (ANum 0))) DO
        X ::= AMinus (AId X) (ANum 1);;
...
Theorem slow_ass_correct : slow_ass.
Proof. unfold slow_ass. intros.

apply hoare_consequence_pre with (P' :=
  (fun st => st X + 0 = m)).
Focus 2. intro. intros. simpl. omega.

eapply hoare_seq.
Focus 2. eapply hoare_consequence_pre.
Focus 1. eapply hoare_asgn with 
  (Q := fun st => st X + st Y = m).
intro; intros. unfold assn_sub. simpl. unfold t_update.
simpl. assumption.

apply hoare_consequence_post with
  (fun st => st X +  st Y = m /\  ~ bassn (BNot (BEq (AId X) (ANum 0))) st).
Focus 2. unfold bassn. simpl. unfold not.
intro; intros. destruct H. destruct (NPeano.Nat.eqb_spec (st X) 0).
rewrite e in H. omega. exfalso. apply H0. reflexivity.

apply hoare_while. eapply hoare_seq.
Focus 1. apply hoare_asgn. unfold assn_sub.
simpl. unfold hoare_triple. intros. inversion H; subst.
unfold t_update; simpl. destruct H0.
unfold bassn in H1.
simpl in H1. rewrite plus_assoc. 
assert (forall x y, (x <> 0) -> x - 1 + y +1 = x + y).
{ induction x; intros. unfold not in H2. elim H2. reflexivity.
simpl. omega. }
rewrite <- H0. apply H2. apply negb_true_iff in H1.
apply beq_nat_false_iff in H1. assumption.
Qed.
(*That really was slow ass*)
...
Definition slow_add : forall n m,
...
apply hoare_consequence_pre with
  (P' := fun st => st X + st Z = n + m).
Focus 2. intro; intros. omega.

apply hoare_consequence_post with
  (Q' := fun st => st X + st Z = n + m /\ ~ (bassn (BNot (BEq (AId X) (ANum 0))) st)).
Focus 2. intro; intros. destruct H. unfold bassn in H0.
apply not_true_is_false in H0. simpl in H0.
apply negb_false_iff in H0. destruct (NPeano.Nat.eqb_spec (st X) 0). omega.
inversion H0.

apply hoare_while. eapply hoare_seq.
Focus 1. apply hoare_asgn. (*This move supplied preconditions*)
 unfold hoare_triple.
intros. destruct H0. inversion H; subst.
simpl. unfold assn_sub. unfold t_update; simpl.
assert (forall x y, (x <> 0) -> x - 1 + y +1 = x + y).
{ induction x; intros. unfold not in H2. elim H2. reflexivity.
simpl. omega. }
rewrite plus_assoc. rewrite <- H0. apply H2.
unfold bassn in H1. simpl in H1. apply negb_true_iff in H1.
destruct (NPeano.Nat.eqb_spec (st X) 0). inversion H1. assumption.
Qed.
...
Focus 2. intro. intros. rewrite H; reflexivity.

apply hoare_consequence_post with 
  (Q' := fun st => parity (st X) = parity m /\ ~ (bassn (BLe (ANum 2) (AId X)) st)).
Focus 2. intro; intros. destruct H. rewrite <- H.
symmetry. apply parity_lt_2. unfold bassn in H0.
apply not_true_is_false in H0. simpl in H0.
intro. destruct (st X). { inversion H1. } destruct n.
omega. inversion H0.

apply hoare_while.

eapply hoare_consequence_pre.
Focus 1. apply hoare_asgn.

unfold bassn, assn_sub. simpl. intro; intros.
destruct H. destruct (st X). inversion H0.
destruct n. inversion H0. simpl.
unfold t_update; simpl. rewrite <- minus_n_O.
replace n with (S (S n) - 2). 
Focus 2. simpl. omega.
rewrite <- H. apply parity_ge_2. omega.
Qed.
...
(** Recall that [n!] denotes the factorial of [n] (i.e., [n! =
  ...
  Y ::= 1;;
    {{          Y = mutation (m, m - X)             }}
  WHILE X <> 0
  DO   {{   Y = mutation (m, m - X)  /\ X <> 0   }} ->>
       {{     Y * X = mutation (m, m - (X - 1))   /\  X <> 0 }}
     Y ::= Y * X;;
       {{     Y = mutation (m, m - (X - 1)) /\ X <> 0        }}
     X ::= X - 1
       {{         Y = mutation (m, m - X) /\ X <> 0           }}
  END
    {{   Y = mutation (m, m - X)    /\ X = 0               }} ->>
    {{ Y = m! }}
*)
...
(** Fill in valid decorations for the following program.
  ...
  {{ True }} ->>
  {{    0 = min (a - a) (b - a) \/  0 = min (a - b) (b - b)      }}
  X ::= a;;
  {{   0 = min (a - X) (b - X) \/  0 = min (a - b) (b - b)          }}
  Y ::= b;;
  {{   0 = min (a - X) (b - X) \/  0 = min (a - Y) (b - Y)   }}
  Z ::= 0;;
  {{   Z = min (a - X) (b - X) \/  Z = min (a - Y) (b - Y)  }}
  WHILE (X <> 0 /\ Y <> 0) DO
  {{   (Z = min (a - X) (b - X) \/  Z = min (a - Y) (b - Y)) Z = 0 /\ GUARD   }} ->>
  {{   (Z + 1 = min (a - (X - 1)) ( b - (X - 1)) \/
  Z + 1 = min (a - (Y - 1)) (b - (Y -1))) /\ GUARD }}
  X := X - 1;;
  {{   Z + 1 = min (a - X) (b - X) \/
  Z + 1 = min (a - (Y - 1)) (b - (Y -1))) /\ GUARD }}
  Y := Y - 1;;
  {{        Z + 1 = min (a - X) (b - X) \/
  Z + 1 = min (a - Y) (b - Y)) /\ GUARD     }}
  Z := Z + 1
  {{       Z = min (a - X) (b - X) \/
  Z = min (a - Y) (b - Y)) /\ GUARD   }}
  END
...
(*
  {{ True }}
    ...
    {{ Y = 2 ^ (X + 1) - 1 /\ X = m }} ->>
    {{ Y = 2 ^ (m + 1) - 1 }}
(*Whatever, the algorithm is self-explanatory*)
*)
(** [] *)
...
- inversion H; subst. unfold t_update; simpl. omega.
- intro; intros. 
remember (X ::= APlus (AId Y) (ANum 1)) as c eqn:Hc.
assert (c /st \\ t_update st X (st Y + 1)).
{ rewrite Hc. constructor. reflexivity. }

apply H in H1.
Focus 2. assumption.
unfold t_update in H1; simpl in H1. omega. Qed.
...
- apply hoare_asgn.
- intro; intros. intro. intros. unfold assn_sub.
...
unfold havoc_pre. intros. unfold assn_sub.
unfold hoare_triple in H. apply H with st.
